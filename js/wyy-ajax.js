...
function sign(){
	$("#sign").html("请稍等正在签到...");
    xiha.postData("wyy.php?do=sign","r="+Math.random(1),function(d){
       if(d.code==200){$("#sign").html("签到成功，经验+"+d.point);
       }else{$("#sign").html("签到失败，原因："+d.msg);}
    });
}
function daka(){
$("#daka").html("请耐心等待半分钟...");
    xiha.postData("wyy.php?do=daka","r="+Math.random(1),function(d){
       if(d.code==200){$("#daka").html("打卡成功，听歌"+d.count+"首");
       }else{$("#daka").html("未知错误");}
    });
}
function check(){
    xiha.postData("wyy.php?do=check","r="+Math.random(1),function(d){
...
function login(uin,pwd){
	...
	xiha.postData(loginurl,"uin="+uin+"&pwd="+md5(pwd)+"&r="+Math.random(1), function(d) {
		if(d.code ==200){
			...
			$('#submit').hide();
			$('#shigeto').show();
			$('#load').html('<div class="list-group-item list-group-item-info"><div class="alert alert-success" style="text-align: center;font-weight: bold;">用户：'+decodeURIComponent(d.profile.nickname)+' 登录成功！</div><button type="button" id="sign" class="btn btn-danger btn-block" style="background:#FF6A6A;">签 到</button><br/><button type="button" id="daka" class="btn btn-danger btn-block" style="background:#FF6A6A;">打 卡</button></div>');
			$('#sign').click(function(){sign();});
			$('#daka').click(function(){daka();});
		}else if(d.code ==502){
			$('#load').html('登录失败，账号密码填写错误！');
